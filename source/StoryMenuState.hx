...
class StoryMenuState extends MusicBeatState
{
	...
	{	
		...
		bg.frames = Paths.getSparrowAtlas('cat/bgSm', 'shared');
		bg.animation.addByPrefix('cet', "cet", 24);
		bg.scrollFactor.set(0.8, 0.9);
		bg.scale.set(6, 6);
		bg.antialiasing = true;
		bg.screenCenter();
		bg.animation.play('cet');
		add(bg);
		...
		override public function update(elapsed:Float)
		{
			...
			if (controls.ACCEPT)
			{
				if (oneclick)
				{
					...
					PlayState.storyPlaylist = ['grrrgarden', 'wacky'];
					...
					video.playMP4(Paths.video('HolyCrap'), new PlayState()); 
					});
