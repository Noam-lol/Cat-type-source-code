...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'cet':
				{
					defaultCamZoom = 0.9;
					curStage = 'cet';
					var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('cat/bg', 'shared'));
					bg.screenCenter();
					bg.antialiasing = true;
					bg.scrollFactor.set(0, 0);
					add(bg);

					var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('cat/ground', 'shared'));
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
					stageFront.antialiasing = true;
					stageFront.scrollFactor.set(0.9, 0.9);
		...
		healthBarBG = new FlxSprite(0, FlxG.height * 0.9).loadGraphic(Paths.image('healthBar', 'preload'));
		if (PlayStateChangeables.useDownscroll)
			healthBarBG.y = 50;
		...
		healthBar.createFilledBar(0xFF808080, 0xFFFF0000);
		// healthBar
		add(healthBar);
...
if (isStoryMode)
{
				...
				if (isStoryMode)
					{
						...
						if (storyPlaylist.length <= 0)
						{
							...
							FlxG.sound.music.stop();
							vocals.stop();
							if (SONG.song.toLowerCase() == 'grrrgarden')
							{
								FlxG.switchState(new VideoState('assets/videos/lol.webm', new PlayState()));
								trace("LOLOLOLOL");
							}
							else if (curSong.toLowerCase() == 'wacky')
							{
	...
	*/
	...
	function cetImage()
		{
			trace ('lol');
			var lolShite:FlxSprite = new FlxSprite(FlxG.random.int(300, 1077), FlxG.random.int(0, 622));
			lolShite.loadGraphic(Paths.image('cat/random/cet' + FlxG.random.int(6,9), 'shared'));
			lolShite.setGraphicSize(FlxG.width, FlxG.height);
			lolShite.screenCenter();
			lolShite.cameras = [camHUD];
			add(lolShite);
			...
			new FlxTimer().start(0.5, function(tmr:FlxTimer)
			{
				FlxG.sound.play(Paths.sound('speech'), 1);
				remove(lolShite);
			});
	}

	function ppBop(pp:Int = 0)
		{
			var pp:FlxSprite = new FlxSprite(0, 0);
	
			pp.frames = Paths.getSparrowAtlas('cat/catBop2', 'shared');
			pp.setGraphicSize(FlxG.width, FlxG.height);
			pp.screenCenter();
			pp.cameras = [camHUD];
			
			pp.animation.addByPrefix('lolBop','cat bop2 instance 1',24, true);
			add(pp);

			if (pp.alpha != 0)
				pp.alpha = FlxG.random.float(0.91, 0.95);

			pp.animation.play('lolBop');

			pp.animation.finishCallback = function(ppp:String)
			{
				trace('lolBop');
				remove(pp);
			}
	...
	override function beatHit()
	{
		...
		if (dad.curCharacter == 'cat-type-beat' && SONG.song.toLowerCase() == 'grrrgarden')
			{
				if (curStep == 696)
				{
					ppBop();
